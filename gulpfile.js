...
_gulp2['default'].task('images', function () {
    return _gulp2['default'].src('app/images/**/*').pipe($['if']($['if'].isFile, $.cache($.imagemin({
        ...
        console.log(err);
        this.end();
    }))).pipe($.rev()).pipe(_gulp2['default'].dest('dist/images')).pipe($.rev.manifest()).pipe(_gulp2['default'].dest('dist/rev/images'));
});

_gulp2['default'].task('fonts', function () {
    return _gulp2['default'].src('app/fonts/**/*').pipe(_gulp2['default'].dest('.tmp/fonts')).pipe($.rev()).pipe(_gulp2['default'].dest('dist/fonts')).pipe($.rev.manifest()).pipe(_gulp2['default'].dest('dist/rev/fonts'));
});
